    ...
    <div className="w-full flex flex-col gap-6">
      ...
      {selectedImage && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 backdrop-blur-sm">
          ...
          <div className="absolute top-0 inset-x-0 p-4 flex justify-between items-center bg-gradient-to-b from-black/50 to-transparent z-10">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center bg-white/10 rounded-lg backdrop-blur-md border border-white/20 mr-4 overflow-hidden">
                <button onClick={handleZoomOut} className="p-2 text-white hover:bg-white/20 transition disabled:opacity-50" disabled={zoomLevel <= 0.5}>
                ...
                <button onClick={handleResetZoom} className="px-2 text-white hover:bg-white/20 transition text-xs font-bold" title="Reset Zoom">
                  R
                </button>
                <button onClick={handleZoomIn} className="p-2 text-white hover:bg-white/20 border-l border-white/10 transition disabled:opacity-50" disabled={zoomLevel >= 3}>
          ...
          {/* Image Container */}
          <div className="relative w-full h-full flex items-center justify-center p-8 overflow-auto" onClick={(e) => {
            ...
            <img 
              ...
              style={{ transform: `scale(${zoomLevel})`, cursor: zoomLevel > 1 ? 'grab' : 'auto' }}
              draggable={false}
            />
          </div>
        </div>
