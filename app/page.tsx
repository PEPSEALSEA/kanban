...
export default function StudyFlow() {
  ...
  const fetchHomework = useCallback(async (email?: string) => {
    ...
    try {
      ...
      const url = `${GAS_WEB_APP_URL}?action=${action}${email ? `&email=${encodeURIComponent(email)}` : ''}`;
      const response = await fetch(url);
      const result = await response.json();
      if (result.success) {
        setAllHomework(result.data);
      }
    } catch (error) {
  ...
  const handleLogout = () => {
    ...
    setAllHomework([]);
    fetchHomework(); // Refresh as public
  };

  const toggleComplete = async (hwId: string, currentStatus?: string) => {
    if (!user) return;
  ...
  const columns = useMemo(() => {
    ...
    const categorized = {
      soon: [] as Homework[], // 3 days
      week: [] as Homework[], // 7 days
      backlog: [] as Homework[] // งานดองเค็ม
    };
    ...
    // Sort: Not done on top, Done at bottom with less opacity
      ...
      <header style={{ padding: '1rem 2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginLeft: '1rem' }}>
            ...
            <h1 style={{ fontSize: '1.5rem', fontWeight: 800, background: 'linear-gradient(to right, #6366f1, #f43f5e)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>StudyFlow</h1>
          </div>
        </div>

        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          {isAdmin && (
            <Link href="/admin" className="glass" style={{ padding: '0.5rem 1rem', borderRadius: '0.75rem', background: 'rgba(244, 63, 94, 0.1)', color: '#f43f5e', fontSize: '0.75rem', fontWeight: 600, border: '1px solid rgba(244, 63, 94, 0.2)' }}>
              Admin Console
            </Link>
          )}
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div>
                        ...
                        <h4 style={{ margin: '0.75rem 0 0.5rem', textDecoration: isDone ? 'line-through' : 'none' }}>{hw.title}</h4>
                      </div>

                      {user && (
                        <div
                          onClick={() => toggleComplete(hw.id, hw.my_status)}
                          style={{
                            width: '24px',
                            height: '24px',
                            borderRadius: '6px',
                    ...
                    <p className="card-desc" style={{ fontSize: '0.85rem' }}>{hw.description}</p>

                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem', fontSize: '0.75rem' }}>
                      <div style={{ color: 'var(--accent)', fontWeight: 600 }}>Due: {new Date(hw.deadline).toLocaleDateString('th-TH')}</div>
                      {hw.link_work && (
                        <a href={hw.link_work} target="_blank" rel="noreferrer" style={{ color: 'var(--primary)', textDecoration: 'none' }}>🔗 View Work</a>
                      )}
                    </div>
                  </div>
                );
